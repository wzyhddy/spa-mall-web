  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
        <el-input v-model="query.name" clearable size="small" placeholder="菜单名称模糊搜索" style="width: 200px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="580px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item v-show="form.type.toString() !== '2'" label="外链菜单" prop="isLink">
        ...
        <el-form-item v-if="form.type.toString() !== '2'" label="菜单名称" prop="name">
          <el-input v-model="form.name" :style=" form.type.toString() === '0' ? 'width: 450px' : 'width: 178px'" placeholder="菜单标题" />
        </el-form-item>
        <el-form-item v-if="form.type.toString() === '2'" label="按钮名称" prop="name">
          <el-input v-model="form.name" placeholder="按钮名称" style="width: 178px;" />
        </el-form-item>
        <el-form-item v-show="form.type.toString() !== '0'" label="权限标识" prop="permission">
          <el-input v-model="form.permission" :disabled="form.isLink === 1" placeholder="权限标识" style="width: 178px;" />
        </el-form-item>
        ...
        <el-form-item v-show="form.isLink !== 1 && form.type.toString() === '1'" label="组件名称" prop="componentName">
          <el-input v-model="form.componentName" style="width: 178px;" placeholder="匹配组件内Name字段" />
        </el-form-item>
        <el-form-item v-show="form.isLink !== 1 && form.type.toString() === '1'" label="组件路径" prop="component">
...
// crud交由presenter持有
const defaultForm = { id: null, name: null, sort: 999, path: null, component: null, componentName: null, isLink: false, roles: [], pid: 0, icon: null, hidden: false, type: 0, permission: null }
export default {
  name: 'Menu',
  components: { Treeselect, IconSelect, crudOperation, rrOperation, udOperation, DateRangePicker },
  cruds() {
    return CRUD({ name: '菜单', url: '/v1/menu/searchByPage', crudMethod: { ...crudMenu }})
  },
    ...
    loadMenus({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        crudMenu.getMenusTree(parentNode.id).then(res => {
          const data = res.data
          parentNode.children = data.map(function(obj) {
            if (!obj.leaf) {
              obj.children = null
            }
            return obj
